...
const SquareDiff: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  return (
    <div style={{ background: '#1a1e2e', borderRadius: 10, padding: 16, color: '#e0e0e0', fontFamily: 'sans-serif' }}>
      {/* Sliders */}
      {[['a', a, setA, 1, 12], ['b', safeB, (v: number) => setB(v), 0.5, a - 0.5]].map(([lbl, val, setter, mn, mx]: any) => (
      ...
      <svg viewBox={`0 0 ${SIZE + PAD * 2} ${SIZE + PAD * 2}`} style={{ width: '100%', maxWidth: 260 }}>
        ...
        <text x={ox - 12} y={oy + A / 2} textAnchor="middle" fontSize="11" fill="#aab" fontFamily="sans-serif">a</text>
      </svg>
    </div>
  );
};
