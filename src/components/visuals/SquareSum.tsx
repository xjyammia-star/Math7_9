...
const SquareSum: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const result = +((total) ** 2).toFixed(2);

  const MAX = 14, PAD = 28, SIZE = 180;
  const sc = SIZE / MAX;
  const A = a * sc, B = b * sc;
  const ox = PAD, oy = PAD;
  ...
  const steps = [
    { key: 'a2', label: 'a²' }, { key: 'ab1', label: 'ab (右上)' },
    { key: 'ab2', label: 'ab (左下)' }, { key: 'b2', label: 'b²' },
      ...
      <g key={`${x}-${y}-${fill}`}>
        ...
        {label && w > 18 && h > 14 && (
          <text x={x + w / 2} y={y + h / 2} textAnchor="middle" dominantBaseline="middle"
            fontSize="11" fill="#fff" fontFamily="sans-serif">{label}</text>
        )}
      </g>
    );
  }
  ...
  return (
    <div style={{ background: '#1a1e2e', borderRadius: 10, padding: 16, color: '#e0e0e0', fontFamily: 'sans-serif' }}>
      ...
      <svg viewBox={`0 0 ${SIZE + PAD * 2} ${SIZE + PAD * 2}`} style={{ width: '100%', maxWidth: 260 }}>
        ...
        <text x={ox + A / 2} y={oy - 10} textAnchor="middle" fontSize="11" fill="#aab" fontFamily="sans-serif">a</text>
        <text x={ox + A + B / 2} y={oy - 10} textAnchor="middle" fontSize="11" fill="#aab" fontFamily="sans-serif">b</text>
        <text x={ox - 14} y={oy + A / 2} textAnchor="middle" fontSize="11" fill="#aab" fontFamily="sans-serif">a</text>
        <text x={ox - 14} y={oy + A + B / 2} textAnchor="middle" fontSize="11" fill="#aab" fontFamily="sans-serif">b</text>
      </svg>
    </div>
  );
};
