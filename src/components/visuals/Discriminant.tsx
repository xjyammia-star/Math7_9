...
const Discriminant: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  return (
    <div style={{ background:'#1a1e2e', borderRadius:10, padding:16, color:'#e0e0e0', fontFamily:'sans-serif' }}>
      ...
      <div style={{ fontSize:11, color:'#bbb', margin:'6px 0 4px', lineHeight:1.8 }}>
        ...
        {roots.length > 0 && <div>x = {roots.map(r => r.toFixed(3)).join(', ')}</div>}
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block', background:'#0f1422', borderRadius:8 }}>
        ...
        <text x={10} y={16} fontSize="10" fill={statusColor} fontFamily="sans-serif" fontWeight="bold">Δ={delta.toFixed(1)}</text>
      </svg>
    </div>
  );
};
