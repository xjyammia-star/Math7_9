...
const SquareSumThree: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const a2 = a*a, b2 = b*b, c2 = c*c, ab = a*b, bc = b*c, ac = a*c;

  const W = 300, H = 280;
  const sc = (Math.min(W,H) * 0.78) / total;
  const A = a*sc, B = b*sc, C = c*sc;
  const ox = (W - A - B - C) / 2, oy = (H - A - B - C) / 2;
  ...
  return (
    <div style={{ background:'#1a1e2e', borderRadius:10, padding:16, color:'#e0e0e0', fontFamily:'sans-serif' }}>
      ...
      <div style={{ fontSize:11, color:'#bbb', margin:'6px 0 8px', lineHeight:1.8 }}>
        <div>(a+b+c)² = a²+b²+c²+2ab+2bc+2ac</div>
        <div>= {a2}+{b2}+{c2}+2×{ab}+2×{bc}+2×{ac} = <strong style={{ color:'#7db3ff' }}>{result}</strong></div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block' }}>
        <rect x={ox} y={oy} width={A+B+C} height={A+B+C} fill="none" stroke="#555" strokeWidth="1" strokeDasharray="4 2" />
        {regions.map((r,i) => (
          <g key={i}>
            <rect x={r.x} y={r.y} width={r.w} height={r.h} fill={r.fill} stroke="#333" strokeWidth="0.5" />
            {r.w > 18 && r.h > 14 &&
              <text x={r.x+r.w/2} y={r.y+r.h/2} textAnchor="middle" dominantBaseline="middle"
                fontSize="10" fill="#fff" fontFamily="sans-serif" fontWeight="bold">{r.label}</text>}
          </g>
        ))}
        <text x={ox+A/2} y={oy-8} textAnchor="middle" fontSize="10" fill="#aab" fontFamily="sans-serif">a</text>
        <text x={ox+A+B/2} y={oy-8} textAnchor="middle" fontSize="10" fill="#aab" fontFamily="sans-serif">b</text>
        <text x={ox+A+B+C/2} y={oy-8} textAnchor="middle" fontSize="10" fill="#aab" fontFamily="sans-serif">c</text>
      </svg>
    </div>
  );
};
