    ...
    <div style={{ background:'#1a1e2e', borderRadius:10, padding:16, color:'#e0e0e0', fontFamily:'sans-serif' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
        ...
        <input type="range" min={-8} max={8} step={0.5} value={k}
          onChange={e=>setK(parseFloat(e.target.value))} style={{ flex:1, accentColor:'#5b8dd9' }} />
      ...
      <div style={{ fontSize:12, color:'#bbb', margin:'6px 0 10px', lineHeight:1.8 }}>
        ...
        <div style={{ fontSize:11 }}>
          {k>0?(lang==='zh'?'第一、三象限':'Quadrants 1 & 3'):k<0?(lang==='zh'?'第二、四象限':'Quadrants 2 & 4'):'—'}
        </div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block', background:'#0f1422', borderRadius:8 }}>
        <defs><clipPath id="invClip"><rect x={PAD} y={PAD} width={W-PAD*2} height={H-PAD*2} /></clipPath></defs>
        ...
        {k!==0 && q1.length>1 && <polyline points={q1.join(' ')} fill="none" stroke="#5b8dd9" strokeWidth="2.5" clipPath="url(#invClip)" />}
        {k!==0 && q3.length>1 && <polyline points={q3.join(' ')} fill="none" stroke="#5b8dd9" strokeWidth="2.5" clipPath="url(#invClip)" />}
        <text x={10} y={18} fontSize="10" fill="#7db3ff" fontFamily="sans-serif">k={k}</text>
      </svg>
    </div>
  );
};
