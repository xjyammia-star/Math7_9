...
const SphereVolume: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  // Dynamic: scale so max radius (6) fills ~38% of canvas, smaller r draws smaller
  const MAX_R = 6;
  const R = Math.min(W,H) * 0.38 * (r / MAX_R);
  const nRings = 6;
  ...
  return (
    <div style={{ background:'#1a1e2e', borderRadius:10, padding:16, color:'#e0e0e0', fontFamily:'sans-serif' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
        ...
        <input type="range" min={1} max={MAX_R} step={0.5} value={r}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block', background:'#0f1422', borderRadius:8 }}>
        {/* Reference circle showing max size */}
        <circle cx={cx} cy={cy} r={Math.min(W,H)*0.38} fill="none" stroke="#ffffff08" strokeWidth="1" strokeDasharray="4 3" />
        {/* Sphere fill */}
        ...
        {Array.from({length: nRings-1}, (_,i) => {
          ...
          return <ellipse key={i} cx={cx} cy={y2} rx={ry2} ry={ry2*0.28}
            fill="none" stroke="#5b8dd950" strokeWidth="0.8" />;
        })}
        {/* Equator */}
        <ellipse cx={cx} cy={cy} rx={R} ry={R*0.28}
          fill="none" stroke="#5b8dd9" strokeWidth="1.2" />
        {/* Radius line */}
        ...
        <text x={cx} y={cy+R*0.5+16} textAnchor="middle" fontSize="13" fill="#7db3ff" fontFamily="sans-serif" fontWeight="bold">V={vol}</text>
      </svg>
    </div>
  );
};
