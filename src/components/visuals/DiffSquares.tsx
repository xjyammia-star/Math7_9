...
const DiffSquares: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  // Dynamic scale: fit a² and b² side-by-side inside W with padding
  const W = 320, H = 260;
  const sc = (W * 0.60) / a;           // big square = 60% of width
  ...
  const steps = [
    ...
    { key: 'diff',      label: zh ? 'L形区域'    : 'L-shape' },
  ...
  return (
    <div style={{ background:'#1a1e2e', borderRadius:10, padding:16, color:'#e0e0e0', fontFamily:'sans-serif' }}>
      ...
      <div style={{ fontSize:11, color:'#bbb', margin:'5px 0 6px', lineHeight:1.8 }}>
        <div>a² − b² = (a+b)(a−b)</div>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:8, flexWrap:'wrap' }}>
        {steps.map(s => (
          <button key={s.key} onClick={() => setStep(s.key)}
            style={{ padding:'2px 10px', borderRadius:20, border:'1px solid', fontSize:11, cursor:'pointer',
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block' }}>
        {step === 'both' && (() => {
          // Place a² on left, b² on right with gap — both within W
          const gap = 16;
          const axL = (W - A - B - gap) / 2;
          const bxL = axL + A + gap;
          return <>
            <rect x={axL} y={(H-A)/2} width={A} height={A} fill="rgba(200,100,140,0.6)" stroke="#a06" strokeWidth="1" />
            <text x={axL+A/2} y={(H-A)/2+A/2} textAnchor="middle" dominantBaseline="middle"
              fontSize="14" fill="#fff" fontFamily="sans-serif" fontWeight="bold">a²</text>
            <rect x={bxL} y={(H-B)/2} width={B} height={B} fill="rgba(80,180,100,0.6)" stroke="#0a5" strokeWidth="1" />
            <text x={bxL+B/2} y={(H-B)/2+B/2} textAnchor="middle" dominantBaseline="middle"
              fontSize="12" fill="#fff" fontFamily="sans-serif" fontWeight="bold">b²</text>
          </>;
        })()}
        ...
        {step === 'diff' && <>
          ...
          <text x={W/2} y={H-12} textAnchor="middle" fontSize="11" fill="#7db3ff" fontFamily="sans-serif">
            {zh ? '蓝色L形' : 'Blue L-shape'} = a²−b²
          </text>
        </>}
        ...
        {step === 'rearrange' && (() => {
          ...
          return <>
            <rect x={rx} y={ry} width={rw} height={rh}
              fill="rgba(80,140,220,0.75)" stroke="#26c" strokeWidth="1.5" />
            <text x={rx+rw/2} y={ry+rh/2} textAnchor="middle" dominantBaseline="middle"
              fontSize="12" fill="#fff" fontFamily="sans-serif" fontWeight="bold">(a+b)×(a−b)</text>
            <text x={rx+rw/2} y={ry-10} textAnchor="middle" fontSize="11" fill="#aab" fontFamily="sans-serif">
              a+b = {+(a+safeB).toFixed(1)}
            </text>
            <text x={rx-8} y={ry+rh/2} textAnchor="end" fontSize="11" fill="#aab" fontFamily="sans-serif">
              a−b={+(a-safeB).toFixed(1)}
            </text>
          </>;
        })()}
