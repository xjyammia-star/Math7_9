...
// ── New 15 components ────────────────────────────────────────────────────────
import SquareSumThree      from './visuals/SquareSumThree';
import Discriminant        from './visuals/Discriminant';
import CompletingSquare    from './visuals/CompletingSquare';
import ArithmeticSum       from './visuals/ArithmeticSum';
import CompoundInterest    from './visuals/CompoundInterest';
import TrapezoidArea       from './visuals/TrapezoidArea';
import SphereVolume        from './visuals/SphereVolume';
import PyramidVolume       from './visuals/PyramidVolume';
import SimilarityRatio     from './visuals/SimilarityRatio';
import DistanceFormula     from './visuals/DistanceFormula';
import MidpointFormula     from './visuals/MidpointFormula';
import Variance            from './visuals/Variance';
import ClassicalProbability from './visuals/ClassicalProbability';
import PythagoreanTriples  from './visuals/PythagoreanTriples';
import SineCosineRule      from './visuals/SineCosineRule';

const VISUAL_MAP: Record<string, React.FC<{ lang: Language }>> = {
  SquareDiff, SquareSum, DiffSquares,
  LinearFunc, QuadraticFunc, InverseFunc,
  Pythagorean, TriangleArea, CircleArea, SectorArea,
  CylinderVolume, ConeVolume,
  TrigBasic, UnitCircle,
  ArithmeticSeq, GeometricSeq, MeanMedian,
  SquareSumThree, Discriminant, CompletingSquare,
  ArithmeticSum, CompoundInterest, TrapezoidArea,
  SphereVolume, PyramidVolume,
  SimilarityRatio, DistanceFormula, MidpointFormula,
  Variance, ClassicalProbability, PythagoreanTriples,
  SineCosineRule,
};
  ...
  <span
    ...
    dangerouslySetInnerHTML={{
      __html: latex
        ...
        .replace(/\\bar\{x\}/g, 'x̄').replace(/\\sum/g, 'Σ').replace(/\\sqrt\{([^}]+)\}/g, '√($1)')
        .replace(/\\Delta/g, 'Δ').replace(/\\sigma/g, 'σ').replace(/\\left\(/g, '(').replace(/\\right\)/g, ')')
        .replace(/\\text\{([^}]+)\}/g, '$1')
        .replace(/_\{([^}]+)\}/g, '<sub>$1</sub>').replace(/\{|\}/g, ''),
    }}
  />
);
...
const FormulaVisualizer: React.FC<Props> = ({ lang }) => {
  ...
  const totalCount = FORMULA_CATEGORIES.reduce((s, c) => s + c.formulas.length, 0);
    ...
    return (
      <motion.div key="detail" initial={{ opacity:0, x:30 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-30 }}
        ...
        <div className="px-6 py-4 border-b border-[var(--color-brand-border)] bg-[var(--color-brand-card)]/40 flex items-center gap-4">
          <button onClick={() => setSelectedFormula(null)}
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="flex flex-col lg:flex-row gap-0 h-full">
            <div className="lg:flex-1 flex items-start justify-center p-4 border-b lg:border-b-0 lg:border-r border-[var(--color-brand-border)] overflow-y-auto">
            ...
            <div className="lg:w-80 p-6 flex flex-col gap-6 shrink-0">
              ...
              <div>
                ...
                <p className="text-sm text-slate-300 leading-relaxed">{selectedFormula.explanation[lang]}</p>
              </div>
              <div className="p-3 rounded-xl bg-amber-900/10 border border-amber-500/20">
                ...
                <p className="text-[11px] text-amber-200/70 leading-relaxed">
                  {lang === 'zh'
                    ...
                    : 'Drag the sliders to change parameter values and watch the diagram update in real time. Use step buttons to walk through the derivation.'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="ml-auto flex gap-2">
            {Object.entries(DIFF_LABELS).map(([k, v]) => (
              <span key={k} className="text-[10px] font-bold px-2 py-1 rounded-full"
        ...
        {FORMULA_CATEGORIES.map(cat => {
          ...
          return (
            <div key={cat.id} className="border border-[var(--color-brand-border)] rounded-2xl overflow-hidden">
              <button onClick={() => setExpandedCategory(isOpen ? null : cat.id)}
                  ...
                  <motion.div initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4 bg-[var(--color-brand-bg)]/40">
                      {cat.formulas.map(formula => {
                        ...
                        return (
                          <motion.button key={formula.id} whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }}
