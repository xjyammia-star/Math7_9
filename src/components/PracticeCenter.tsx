...
import { Concept, Language, Difficulty, Grade } from '../types';
...
const PracticeCenter: React.FC<PracticeCenterProps> = ({ concept, lang, searchQuery, onSelectTopic }) => {
  ...
  const t = {
    ...
    en: {
      ...
      desc: 'Please select or input a topic on the left. AI will design exercises based on your selected difficulty and grade.',
  ...
  const handleGenerate = async () => {
    ...
    if (!activeConcept && searchQuery && onSelectTopic) {
      setLoading(true);
      try {
        await onSelectTopic(searchQuery);
      } catch (e) {}
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="h-full overflow-y-auto bg-[var(--color-brand-bg)]">
      <div className="max-w-6xl mx-auto p-8 md:p-12 space-y-10">
        <header>
          <h2 className="text-3xl font-bold text-white flex items-center gap-3">
            <ClipboardList className="w-8 h-8 text-[var(--color-brand-accent)]" />
            {lang === 'zh' ? '专项习题训练' : 'Practice Forge'}
          </h2>
          <p className="text-slate-500 text-sm mt-3 leading-relaxed max-w-xl">{t.desc}</p>
        </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex items-end gap-3">
              <button 
                ...
                disabled={loading}
                className="flex-1 h-[42px] gold-btn rounded-xl text-sm transition-all shadow-xl shadow-amber-900/10 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading ? (
        ...
        <AnimatePresence mode="wait">
          ...
          {exercises ? (
            ...
            >
              <div className="dark-card rounded-3xl p-10 overflow-hidden relative">
                <div className="markdown-body prose prose-invert max-w-none prose-sm prose-p:leading-relaxed relative z-10 
                  prose-img:rounded-2xl prose-img:border prose-img:border-[var(--color-brand-border)] prose-img:bg-slate-900/50 prose-img:p-4 prose-img:max-h-[400px] prose-img:mx-auto">
                  <ReactMarkdown 
                ...
                <div className="mt-16 flex flex-wrap gap-4 border-t border-[var(--color-brand-border)] pt-8">
                  <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-bold transition-all border uppercase tracking-widest ${
                      isGuiding 
                      ? 'bg-[var(--color-brand-accent)] text-white border-[var(--color-brand-accent)] shadow-lg shadow-[var(--color-brand-accent)]/20' 
              ...
              <div className="bg-amber-900/10 p-8 rounded-3xl border border-amber-900/20 flex gap-6 backdrop-blur-sm">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed font-medium">
                    {showSolution 
                      ? (lang === 'zh' 
                         ? '如果你对答案有疑问，或者想了解推导过程，请点击“引导解题”开始交互学习。' 
                         : 'If you have questions about the answer or want to learn the derivation, click "Guide Me" to start.')
                      : (lang === 'zh' 
                         ? '本工具支持直接查看答案，也支持通过“引导解题”进行深度思考。建议先独立思考再核对。' 
                         : 'This tool supports both direct answers and guided thinking. Try solving independently before checking.')
                    }
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <h3 className="text-xl font-bold text-slate-400 mb-2">{concept ? concept.title[lang] : t.selectPrompt}</h3>
